import {
  require_react_dom
} from "./chunk-KVLTXS75.js";
import {
  require_react
} from "./chunk-OIIC662F.js";
import {
  __commonJS,
  __objRest,
  __publicField,
      ...
      while (Object.keys(encoder2.deferred).length > 0) {
        for (const [deferredId, deferred] of Object.entries(encoder2.deferred)) {
          if (seenPromises.has(deferred))
            continue;
          seenPromises.add(encoder2.deferred[Number(deferredId)] = raceSignal(deferred, encoder2.signal).then((resolved) => {
...
function convertRoutesToDataRoutes(routes, mapRouteProperties2, parentPath = [], manifest = {}) {
  return routes.map((route, index) => {
    ...
    if (isIndexRoute(route)) {
      let indexRoute = __spreadProps(__spreadValues(__spreadValues({}, route), mapRouteProperties2(route)), {
        id
      });
      manifest[id] = indexRoute;
      return indexRoute;
    } else {
      let pathOrLayoutRoute = __spreadProps(__spreadValues(__spreadValues({}, route), mapRouteProperties2(route)), {
        id,
        children: void 0
      });
...
var redirect = (url, init = 302) => {
  ...
  return new Response(null, __spreadProps(__spreadValues({}, responseInit), { headers }));
};
...
function createRouter(init) {
  ...
  let future = __spreadValues({}, init.future);
  ...
  function updateState(newState, opts = {}) {
    state = __spreadValues(__spreadValues({}, state), newState);
  ...
  function completeNavigation(location, newState, { flushSync: flushSync2 } = {}) {
    ...
    updateState(__spreadProps(__spreadValues({}, newState), {
      actionData,
      loaderData,
  ...
  async function navigate(to, opts) {
    ...
    nextLocation = __spreadValues(__spreadValues({}, nextLocation), init.history.encodeLocation(nextLocation));
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    let matches = (opts == null ? void 0 : opts.initialHydration) && state.matches && state.matches.length > 0 && !initialMatchesIsFOW ? state.matches : matchRoutes(routesToUse, location, basename);
    let flushSync2 = (opts && opts.flushSync) === true;
    ...
    if (opts && opts.pendingError) {
      ...
      let actionResult = await handleAction(request, location, opts.submission, matches, fogOfWar.active, { replace: opts.replace, flushSync: flushSync2 });
    ...
    pendingNavigationController = null;
    completeNavigation(location, __spreadProps(__spreadValues({
  ...
  async function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace2, initialHydration, flushSync2, pendingActionResult) {
    ...
    if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
      ...
      completeNavigation(location, __spreadValues(__spreadValues({
        matches,
        loaderData: {},
        errors: pendingActionResult && isErrorResult(pendingActionResult[1]) ? { [pendingActionResult[0]]: pendingActionResult[1].error } : null
      }, getActionDataForCommit(pendingActionResult)), updatedFetchers2 ? { fetchers: new Map(state.fetchers) } : {}), { flushSync: flushSync2 });
      return { shortCircuited: true };
    }
    if (shouldUpdateNavigationState) {
      let updates = {};
  ...
  async function fetch2(key, routeId, href2, opts) {
    ...
    let flushSync2 = (opts && opts.flushSync) === true;
    let routesToUse = inFlightDataRoutes || dataRoutes;
    let normalizedPath = normalizeTo(state.location, state.matches, basename, href2, routeId, opts == null ? void 0 : opts.relative);
    ...
    if (!matches) {
      setFetcherError(key, routeId, getInternalRouterError(404, { pathname: normalizedPath }), { flushSync: flushSync2 });
      return;
    }
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync2, preventScrollReset, submission) {
    ...
    if (isFogOfWar) {
      ...
      if (discoverResult.type === "aborted") {
        ...
        setFetcherError(key, routeId, getInternalRouterError(404, { pathname: path }), { flushSync: flushSync2 });
        return;
      } else {
  ...
  async function handleFetcherLoader(key, routeId, path, match, matches, isFogOfWar, flushSync2, preventScrollReset, submission) {
    let existingFetcher = state.fetchers.get(key);
    updateFetcherState(key, getLoadingFetcher(submission, existingFetcher ? existingFetcher.data : void 0), { flushSync: flushSync2 });
    let abortController = new AbortController();
  ...
  async function startRedirectNavigation(request, redirect2, isNavigation, {
    ...
    if (redirectPreserveMethodStatusCodes.has(redirect2.response.status) && activeSubmission && isMutationMethod(activeSubmission.formMethod)) {
      await startNavigation(redirectNavigationType, redirectLocation, {
        submission: __spreadProps(__spreadValues({}, activeSubmission), {
          formAction: location
        }),
...
function createStaticHandler(routes, opts) {
  ...
  async function query(request, {
    ...
    return __spreadValues({ location, basename }, result);
  }
  ...
  async function queryImpl(request, location, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch) {
    ...
    try {
      ...
      return isResponse(result) ? result : __spreadProps(__spreadValues({}, result), {
        actionData: null,
        actionHeaders: {}
      });
    } catch (e) {
  ...
  async function submit(request, matches, actionMatch, requestContext, dataStrategy, skipLoaderErrorBubbling, isRouteRequest) {
    ...
    if (isErrorResult(result)) {
      ...
      return __spreadProps(__spreadValues({}, context2), {
        statusCode: isRouteErrorResponse(result.error) ? result.error.status : result.statusCode != null ? result.statusCode : 500,
        actionData: null,
        actionHeaders: __spreadValues({}, result.headers ? { [actionMatch.route.id]: result.headers } : {})
      });
    }
    let context = await loadRouteData(loaderRequest, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, null);
    return __spreadProps(__spreadValues(__spreadProps(__spreadValues({}, context), {
      actionData: {
...
function getStaticContextFromError(routes, context, error) {
  let newContext = __spreadProps(__spreadValues({}, context), {
...
function getMatchesToLoad(history, state, matches, submission, location, initialHydration, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult) {
  ...
  let navigationMatches = boundaryMatches.filter((match, index) => {
    ...
    return shouldRevalidateLoader(match, __spreadProps(__spreadValues({
      currentUrl,
  ...
  fetchLoadMatches.forEach((f, key) => {
    ...
    if (fetchRedirectIds.has(key)) {
      ...
      shouldRevalidate = shouldRevalidateLoader(fetcherMatch, __spreadProps(__spreadValues({
        currentUrl,
...
function patchRoutesImpl(routeId, children, routesToUse, manifest, mapRouteProperties2) {
  ...
  let newRoutes = convertRoutesToDataRoutes(uniqueChildren, mapRouteProperties2, [routeId || "_", "patch", String((childrenToPatch == null ? void 0 : childrenToPatch.length) || "0")], manifest);
...
async function loadLazyRouteModule(route, mapRouteProperties2, manifest) {
  ...
  Object.assign(routeToUpdate, __spreadProps(__spreadValues({}, mapRouteProperties2(routeToUpdate)), {
    lazy: void 0
  }));
}
...
async function callDataStrategyImpl(dataStrategyImpl, type, state, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties2, requestContext) {
  ...
  let dsMatches = matches.map((match, i) => {
    ...
    let resolve = async (handlerOverride) => {
      ...
      return shouldLoad ? callLoaderOrAction(type, request, match, loadRoutePromise, handlerOverride, requestContext) : Promise.resolve({ type: "data", result: void 0 });
    };
...
async function convertDataStrategyResultToDataResult(dataStrategyResult) {
  ...
  if (type === "error") {
    if (isDataWithResponseInit(result)) {
      ...
      return {
        type: "error",
        error: new ErrorResponseImpl(((_c = result.init) == null ? void 0 : _c.status) || 500, void 0, result.data),
  ...
  revalidatingFetchers.forEach((rf) => {
    ...
    if (controller && controller.signal.aborted) {
      ...
      if (!(errors && errors[boundaryMatch.route.id])) {
        errors = __spreadProps(__spreadValues({}, errors), {
...
function stripHashFromPath(path) {
  ...
  return createPath(__spreadProps(__spreadValues({}, parsedPath), { hash: "" }));
}
...
var RenderErrorBoundary = class extends React2.Component {
  ...
  render() {
    return this.state.error !== void 0 ? React2.createElement(RouteContext.Provider, { value: this.props.routeContext }, React2.createElement(RouteErrorContext.Provider, {
...
function _renderMatches(matches, parentMatches = [], dataRouterState = null, future = null) {
  ...
  return renderedMatches.reduceRight((outlet, match, index) => {
    ...
    return dataRouterState && (match.route.ErrorBoundary || match.route.errorElement || index === 0) ? React2.createElement(RenderErrorBoundary, {
...
function useBlocker(shouldBlock) {
  ...
  let blockerFunction = React2.useCallback((arg) => {
    ...
    return shouldBlock({
      currentLocation: __spreadProps(__spreadValues({}, currentLocation), {
        pathname: stripBasename(currentLocation.pathname, basename) || currentLocation.pathname
      }),
      nextLocation: __spreadProps(__spreadValues({}, nextLocation), {
...
function useNavigateStable() {
  ...
  let navigate = React2.useCallback(async (to, options = {}) => {
    ...
    if (typeof to === "number") {
      ...
      await router2.navigate(to, __spreadValues({ fromRouteId: id }, options));
    }
  }, [router2, id]);
  return navigate;
}
...
var AwaitErrorBoundary = class extends React3.Component {
  ...
  render() {
    ...
    if (!(resolve instanceof Promise)) {
      ...
      promise = resolve.then((data2) => Object.defineProperty(resolve, "_data", { get: () => data2 }), (error) => Object.defineProperty(resolve, "_error", { get: () => error }));
    }
...
async function getKeyedPrefetchLinks(matches, manifest, routeModules) {
  ...
  return dedupeLinkDescriptors(links.flat(1).filter(isHtmlLinkDescriptor).filter((link) => link.rel === "stylesheet" || link.rel === "preload").map((link) => link.rel === "stylesheet" ? __spreadProps(__spreadValues({}, link), { rel: "prefetch", as: "style" }) : __spreadProps(__spreadValues({}, link), { rel: "prefetch" })));
}
...
function StreamTransfer({
  ...
  let scriptTag = value ? React4.createElement("script", {
    nonce,
  ...
  if (done) {
    return React4.createElement(React4.Fragment, null, scriptTag, React4.createElement("script", {
      nonce,
      dangerouslySetInnerHTML: {
    ...
    return React4.createElement(React4.Fragment, null, scriptTag, React4.createElement(React4.Suspense, null, React4.createElement(StreamTransfer, {
      context,
...
var RemixErrorBoundary = class extends React5.Component {
  ...
  render() {
    if (this.state.error) {
      return React5.createElement(RemixRootDefaultErrorBoundary, {
...
function RemixRootDefaultErrorBoundary({
  ...
  console.error(error);
  let heyDeveloper = React5.createElement("script", {
    dangerouslySetInnerHTML: {
      __html: `
        console.log(
  ...
  return React5.createElement(BoundaryShell, {
    ...
    isOutsideRemixApp
  }, React5.createElement("h1", { style: { fontSize: "24px" } }, "Application Error"), React5.createElement("pre", {
    style: {
      padding: "2rem",
...
function BoundaryShell({
  ...
  return React5.createElement("html", { lang: "en" }, React5.createElement("head", null, React5.createElement("meta", { charSet: "utf-8" }), React5.createElement("meta", {
    name: "viewport",
...
function RemixRootDefaultHydrateFallback() {
  return React6.createElement(BoundaryShell, { title: "Loading...", renderScripts: true }, React6.createElement("script", {
    dangerouslySetInnerHTML: {
      __html: `
              console.log(
...
function getRouteComponents(route, routeModule, isSpaMode) {
  ...
  if (route.id === "root" && routeModule.Layout) {
    return __spreadValues(__spreadValues(__spreadValues({}, Component4 ? {
      element: React7.createElement(routeModule.Layout, null, React7.createElement(Component4, null))
    } : { Component: Component4 }), ErrorBoundary ? {
...
function createServerRoutes(manifest, routeModules, future, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId(manifest), spaModeLazyPromise = Promise.resolve({ Component: () => null })) {
  return (routesByParentId[parentId] || []).map((route) => {
    ...
    let dataRoute = __spreadProps(__spreadValues({}, getRouteComponents(route, routeModule, isSpaMode)), {
...
function createClientRoutes(manifest, routeModulesCache, initialState, ssr, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId(manifest), needsRevalidation) {
  return (routesByParentId[parentId] || []).map((route) => {
    ...
    if (routeModule) {
      Object.assign(dataRoute, __spreadProps(__spreadValues(__spreadValues({}, dataRoute), getRouteComponents(route, routeModule, isSpaMode)), {
        handle: routeModule.handle,
      ...
      if (!route.hasClientLoader) {
        ...
        dataRoute.loader = async (args, singleFetch) => {
          ...
          return clientLoader(__spreadProps(__spreadValues({}, args), {
            async serverLoader() {
              preventInvalidServerHandlerCall("loader", route);
              return fetchServerLoader(singleFetch);
            }
      ...
      if (!route.hasClientAction) {
        ...
        dataRoute.action = async (args, singleFetch) => {
          ...
          return clientAction(__spreadProps(__spreadValues({}, args), {
            async serverAction() {
              preventInvalidServerHandlerCall("action", route);
              return fetchServerAction(singleFetch);
            }
      ...
      dataRoute.lazy = async () => {
        ...
        let lazyRoute = __spreadValues({}, mod);
        if (mod.clientLoader) {
          let clientLoader = mod.clientLoader;
          lazyRoute.loader = (args, singleFetch) => clientLoader(__spreadProps(__spreadValues({}, args), {
            async serverLoader() {
              preventInvalidServerHandlerCall("loader", route);
              return fetchServerLoader(singleFetch);
            }
        ...
        if (mod.clientAction) {
          ...
          lazyRoute.action = (args, singleFetch) => clientAction(__spreadProps(__spreadValues({}, args), {
            async serverAction() {
              preventInvalidServerHandlerCall("action", route);
              return fetchServerAction(singleFetch);
            }
        ...
        return __spreadProps(__spreadValues(__spreadValues({}, lazyRoute.loader ? { loader: lazyRoute.loader } : {}), lazyRoute.action ? { action: lazyRoute.action } : {}), {
...
function getShouldRevalidateFunction(route, manifestRoute, ssr, needsRevalidation) {
  ...
  if (!ssr && manifestRoute.hasLoader && !manifestRoute.hasClientLoader) {
    if (route.shouldRevalidate) {
      ...
      return (opts) => fn(__spreadProps(__spreadValues({}, opts), { defaultShouldRevalidate: false }));
    } else {
      return () => false;
    }
  }
  if (ssr && route.shouldRevalidate) {
    let fn = route.shouldRevalidate;
    return (opts) => fn(__spreadProps(__spreadValues({}, opts), { defaultShouldRevalidate: true }));
  }
...
function useFogOFWarDiscovery(router2, manifest, routeModules, ssr, isSpaMode) {
  React8.useEffect(() => {
    ...
    async function fetchPatches() {
      ...
      try {
        await fetchAndApplyManifestPatches(lazyPaths, manifest, routeModules, ssr, isSpaMode, router2.basename, router2.patchRoutes);
      } catch (e) {
...
function Links() {
  ...
  return React9.createElement(React9.Fragment, null, criticalCss ? React9.createElement("style", { dangerouslySetInnerHTML: { __html: criticalCss } }) : null, keyedLinks.map(({ key, link }) => isPageLinkDescriptor(link) ? React9.createElement(PrefetchPageLinks, __spreadValues({ key }, link)) : React9.createElement("link", __spreadValues({ key }, link))));
}
function PrefetchPageLinks(_a) {
  ...
  let matches = React9.useMemo(() => matchRoutes(router2.routes, page, router2.basename), [router2.routes, page, router2.basename]);
  if (!matches) {
    return null;
  }
  return React9.createElement(PrefetchPageLinksImpl, __spreadValues({ page, matches }, dataLinkProps));
}
...
function PrefetchPageLinksImpl(_a) {
  ...
  return React9.createElement(React9.Fragment, null, dataHrefs.map((href2) => React9.createElement("link", __spreadValues({ key: href2, rel: "prefetch", as: "fetch", href: href2 }, linkProps))), moduleHrefs.map((href2) => React9.createElement("link", __spreadValues({ key: href2, rel: "modulepreload", href: href2 }, linkProps))), keyedPrefetchLinks.map(({ key, link }) => React9.createElement("link", __spreadValues({ key }, link))));
}
function Meta() {
  ...
  return React9.createElement(React9.Fragment, null, meta.flat().map((metaProps) => {
    ...
    if ("tagName" in metaProps) {
      let _a = metaProps, { tagName } = _a, rest = __objRest(_a, ["tagName"]);
      if (!isValidMetaTag(tagName)) {
        console.warn(`A meta object uses an invalid tagName: ${tagName}. Expected either 'link' or 'meta'`);
        return null;
      }
      let Comp = tagName;
      return React9.createElement(Comp, __spreadValues({ key: JSON.stringify(rest) }, rest));
    }
    if ("title" in metaProps) {
      return React9.createElement("title", { key: "title" }, String(metaProps.title));
    }
    if ("charset" in metaProps) {
      (_b = metaProps.charSet) != null ? _b : metaProps.charSet = metaProps.charset;
    ...
    if ("script:ld+json" in metaProps) {
      try {
        ...
        return React9.createElement("script", {
    ...
    return React9.createElement("meta", __spreadValues({ key: JSON.stringify(metaProps) }, metaProps));
  }));
}
...
${matches.map((match, routeIndex) => {
  ...
  ${enableFogOfWar ? `window.__reactRouterManifest = ${JSON.stringify(getPartialManifest(manifest, router2), null, 2)};` : ""}
  window.__reactRouterRouteModules = {${matches.map((match, index) => `${JSON.stringify(match.route.id)}:route${index}`).join(",")}};

import(${JSON.stringify(manifest.entry.module)});`;
    return React9.createElement(React9.Fragment, null, React9.createElement("script", __spreadProps(__spreadValues({}, props), {
      suppressHydrationWarning: true,
      dangerouslySetInnerHTML: createHtml(contextScript),
      type: void 0
    })), React9.createElement("script", __spreadProps(__spreadValues({}, props), {
      suppressHydrationWarning: true,
  ...
  return isHydrated ? null : React9.createElement(React9.Fragment, null, !enableFogOfWar ? React9.createElement("link", {
    rel: "modulepreload",
    href: manifest.url,
    crossOrigin: props.crossOrigin
  }) : null, React9.createElement("link", {
    rel: "modulepreload",
    href: manifest.entry.module,
    crossOrigin: props.crossOrigin
  }), dedupe(preloads).map((path) => React9.createElement("link", {
    key: path,
    rel: "modulepreload",
    href: path,
    crossOrigin: props.crossOrigin
...
var Link = React10.forwardRef(function LinkWithRef(_a, forwardedRef) {
  var _b = _a, {
    onClick,
    discover = "render",
    prefetch = "none",
  ...
  let link = React10.createElement("a", __spreadProps(__spreadValues(__spreadValues({}, rest), prefetchHandlers), {
...
var NavLink = React10.forwardRef(function NavLinkWithRef(_a, ref) {
  var _b = _a, {
  ...
  return React10.createElement(Link, __spreadProps(__spreadValues({}, rest), {
...
var Form = React10.forwardRef((_a, forwardedRef) => {
  var _b = _a, {
    discover = "render",
    fetcherKey,
    navigate,
  ...
  return React10.createElement("form", __spreadProps(__spreadValues({
    ref: forwardedRef,
    method: formMethod,
    action: formAction,
...
function ScrollRestoration(_a) {
  ...
  return React10.createElement("script", __spreadProps(__spreadValues({}, props), {
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
...
function useSearchParams(defaultInit) {
  ...
  let location = useLocation();
  let searchParams = React10.useMemo(() => getSearchParamsForLocation(location.search, hasSetSearchParamsRef.current ? null : defaultSearchParamsRef.current), [location.search]);
  let navigate = useNavigate();
...
function useFetcher({
  ...
  let submit = React10.useCallback(async (target, opts) => {
    await submitImpl(target, __spreadProps(__spreadValues({}, opts), {
      navigate: false,
      fetcherKey
    }));
  ...
  let FetcherForm = React10.useMemo(() => {
    let FetcherForm2 = React10.forwardRef((props, ref) => {
      return React10.createElement(Form, __spreadProps(__spreadValues({}, props), { navigate: false, fetcherKey, ref }));
    });
  ...
  let fetcherWithComponents = React10.useMemo(() => __spreadProps(__spreadValues({
    Form: FetcherForm,
    submit,
    load
  }, fetcher), {
...
function useFetchers() {
  ...
  return Array.from(state.fetchers.entries()).map(([key, fetcher]) => __spreadProps(__spreadValues({}, fetcher), {
    key
  }));
}
...
function getScrollRestorationKey(location, matches, basename, getKey) {
  ...
  if (getKey) {
    if (basename !== "/") {
      key = getKey(__spreadProps(__spreadValues({}, location), {
...
function useScrollRestoration({
  ...
  if (typeof document !== "undefined") {
    ...
    React10.useLayoutEffect(() => {
      let disableScrollRestoration = router2 == null ? void 0 : router2.enableScrollRestoration(savedScrollPositions, () => window.scrollY, getKey ? (location2, matches2) => getScrollRestorationKey(location2, matches2, basename, getKey) : void 0);
...
function createStaticHandler2(routes, opts) {
  return createStaticHandler(routes, __spreadProps(__spreadValues({}, opts), {
    mapRouteProperties
  }));
}
function createStaticRouter(routes, context, opts = {}) {
  ...
  return {
    ...
    get future() {
      return __spreadValues({}, opts == null ? void 0 : opts.future);
    },
    get state() {
      return {
        historyAction: "POP",
...
function ServerRouter({
  ...
  context.staticHandlerContext.loaderData = __spreadValues({}, context.staticHandlerContext.loaderData);
  ...
  }, React12.createElement(RemixErrorBoundary, { location: router2.state.location }, React12.createElement(StaticRouterProvider, {
    router: router2,
    context: context.staticHandlerContext,
    hydrate: false
  }))), context.serverHandoffStream ? React12.createElement(React12.Suspense, null, React12.createElement(StreamTransfer, {
    context,
    identifier: 0,
...
function createRoutesStub(routes, context = {}) {
  return function RoutesTestStub({
    ...
    if (routerRef.current == null) {
      ...
      let patched = processRoutes(convertRoutesToDataRoutes(routes, (r) => r), context, remixContextRef.current.manifest, remixContextRef.current.routeModules);
...
function processRoutes(routes, context, manifest, routeModules, parentId) {
  return routes.map((route) => {
    ...
    let newRoute = {
      ...
      action: action ? (args) => action(__spreadProps(__spreadValues({}, args), { context })) : void 0,
      loader: loader ? (args) => loader(__spreadProps(__spreadValues({}, args), { context })) : void 0,
      handle: route.handle,
...
var createKey2 = async (secret, usages) => crypto.subtle.importKey("raw", encoder.encode(secret), { name: "HMAC", hash: "SHA-256" }, false, usages);
...
var createCookie = (name, cookieOptions = {}) => {
  let _a = __spreadValues({
    path: "/",
    sameSite: "lax"
  }, cookieOptions), { secrets = [] } = _a, options = __objRest(_a, ["secrets"]);
...
function createRoutes(manifest, parentId = "", routesByParentId = groupRoutesByParentId2(manifest)) {
  return (routesByParentId[parentId] || []).map((route) => __spreadProps(__spreadValues({}, route), {
...
function createStaticHandlerDataRoutes(manifest, future, parentId = "", routesByParentId = groupRoutesByParentId2(manifest)) {
  return (routesByParentId[parentId] || []).map((route) => {
    ...
    return route.index ? __spreadValues({
      index: true
    }, commonRoute) : __spreadValues({
      caseSensitive: route.caseSensitive,
...
async function singleFetchAction(build, serverMode, staticHandler, request, handlerUrl, loadContext, handleError) {
  try {
    let handlerRequest = new Request(handlerUrl, __spreadValues({
      method: request.method,
      body: request.body,
      headers: request.headers,
      signal: request.signal
...
var createRequestHandler = (build, mode) => {
  ...
  return async function requestHandler(request, loadContext = {}) {
    ...
    let manifestUrl = `${(_a = _build.basename) != null ? _a : "/"}/__manifest`.replace(/\/+/g, "/");
...
async function handleDocumentRequest(serverMode, build, staticHandler, request, loadContext, handleError, criticalCss) {
  ...
  try {
    ...
    entryContext = __spreadProps(__spreadValues({}, entryContext), {
      staticHandlerContext: context,
      serverHandoffString: createServerHandoffString({
        basename: build.basename,
        future: build.future,
...
function RouterProvider2(props) {
  return React14.createElement(RouterProvider, __spreadValues({ flushSync: ReactDOM.flushSync }, props));
}
var ssrInfo = null;
var router = null;
...
function createHydratedRouter() {
  ...
  if (ssrInfo.context.isSpaMode) {
    ...
    hydrationData = __spreadProps(__spreadValues({}, ssrInfo.context.state), {
      loaderData: __spreadValues({}, loaderData)
    });
    let initialMatches = matchRoutes(routes, window.location, (_a = window.__reactRouterContext) == null ? void 0 : _a.basename);
    if (initialMatches) {
...
function HydratedRouter() {
  ...
  let [criticalCss, setCriticalCss] = React22.useState(true ? ssrInfo == null ? void 0 : ssrInfo.context.criticalCss : void 0);
  if (true) {
    if (ssrInfo) {
  ...
  return React22.createElement(React22.Fragment, null, React22.createElement(FrameworkContext.Provider, {
    value: {
  ...
  }, React22.createElement(RemixErrorBoundary, { location }, React22.createElement(RouterProvider2, { router }))), React22.createElement(React22.Fragment, null));
}
export {
  Await,
  BrowserRouter,
